...
public class RailInfo {
    ...
    private String generateID() {
        Object[] props = new Object[]{
                ...
                this.settings.ctrl1Roll,
                this.settings.midRoll,
                this.settings.ctrl2Roll
        };
        String id = Arrays.toString(props);
        if(!placementInfo.placementPosition.equals(
                customInfo.placementPosition) || this.settings.posType != TrackPositionType.FIXED){
    ...
    public RailInfo offset(Vec3i offset) {
        return with(b -> {
            ...
            b.customInfo = b.customInfo != null ?
                           b.customInfo.offset(offset) :
                           null;
        });
    }
    ...
    private class MaterialManager {
        ...
        private boolean checkMaterials(Player player) {
            ...
            for(int i = 0; i < player.getInventory().getSlotCount(); i++) {
                ...
                if(material.apply(stack) && (!stack.is(IRItems.ITEM_RAIL) || new ItemRail.Data(
                        stack).gauge == settings.gauge)){
                    found += stack.getCount();
                }
            }

            if(found < count){
                String example = Arrays.stream(examples).map(ItemStack::getDisplayName).limit(5)
            ...
            if(player.getWorld().isServer){
                ...
                if(tieParts != null){
                    for(TrackDefinition.TrackMaterial tiePart : tieParts) {
                        materials.add(new MaterialManager((int) Math.ceil(builder.costTies() * tiePart.cost),
                                                          tiePart::matches, tiePart.examples(settings.gauge)));
                    }
                }
                if(railParts != null){
                    for(TrackDefinition.TrackMaterial railPart : railParts) {
                        materials.add(new MaterialManager((int) Math.ceil(builder.costRails() * railPart.cost),
                                                          railPart::matches, railPart.examples(settings.gauge)));
                    }
                }
                if(bedParts != null){
                    for(TrackDefinition.TrackMaterial bedPart : bedParts) {
                        materials.add(
                                new MaterialManager((int) Math.ceil(builder.costBed() * bedPart.cost), bedPart::matches,
                                                    bedPart.examples(settings.gauge)));
                    }
                }
    ...
    public static class TagMapper implements cam72cam.mod.serialization.TagMapper<RailInfo> {
        ...
        public TagAccessor<RailInfo> apply(Class<RailInfo> type, String fieldName, TagField tag) {
            return new TagAccessor<>(
                    ...
                    (d, w) -> d.hasKey(fieldName) ?
                              new Mutable(d.get(fieldName)).immutable() :
                              legacy(d)
            );
        }
        ...
        private static RailInfo legacy(TagCompound nbt) {
            ...
            placementInfo = new PlacementInfo(placementInfo.placementPosition, placementInfo.direction,
                                              placementInfo.yaw, null);
            ...
            RailSettings settings = new RailSettings(gauge, "default", type, length, quarters / 4F * 90, 1,
                                                     TrackPositionType.FIXED, type == TrackItems.SLOPE ?
                                                                              TrackSmoothing.NEITHER :
                                                                              TrackSmoothing.BOTH, TrackDirection.NONE,
                                                     railBed, cam72cam.mod.item.ItemStack.EMPTY, false, false, 0, 0, 0);
