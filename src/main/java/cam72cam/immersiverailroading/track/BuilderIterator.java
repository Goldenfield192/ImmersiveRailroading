...
public abstract class BuilderIterator extends BuilderBase implements IIterableTrack {
	...
	public List<VecYawPitchRoll> getRenderData() {
		...
		for (int i = 0; i < points.size(); i++) {
			...
			if (angle != 0) {
				if (direction == TrackDirection.RIGHT) {
					data.add(new VecYawPitchRoll(switchPos.x, switchPos.y, switchPos.z, switchPos.yaw, switchPos.pitch, (1 - angle / 180) * (float) info.settings.gauge.scale(), "RAIL_LEFT"));
					data.add(new VecYawPitchRoll(cur.x, cur.y, cur.z, cur.yaw, cur.pitch, (1 + angle / 180) * (float) info.settings.gauge.scale(), "RAIL_RIGHT"));
				} else {
					data.add(new VecYawPitchRoll(cur.x, cur.y, cur.z, cur.yaw, cur.pitch, (1 - angle / 180) * (float) info.settings.gauge.scale(), "RAIL_LEFT"));
					data.add(new VecYawPitchRoll(switchPos.x, switchPos.y, switchPos.z, switchPos.yaw, switchPos.pitch, (1 + angle / 180) * (float) info.settings.gauge.scale(), "RAIL_RIGHT"));
				}
				data.add(new VecYawPitchRoll(cur.x, cur.y, cur.z, cur.yaw, cur.pitch, "RAIL_BASE"));
			} else {
				data.add(new VecYawPitchRoll(cur.x, cur.y, cur.z, cur.yaw, cur.pitch));
			}
		}
